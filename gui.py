...
class GUI:
    ...
    def updateHighlightedCellStats(self):
        ...
        if cell != None:
            ...
            agent = cell.agent
            if agent != None:
                agentStats = "Agent: {str(agent)} | Age: {agent.age} | Vision: {round(agent.findVision(), 2)} | Movement: {round(agent.findMovement(), 2)} | "
                agentStats += "Sugar: {round(agent.sugar, 2)} | Spice: {round(agent.spice, 2)} | "
                agentStats += "Metabolism: {round(((agent.findSugarMetabolism() + agent.findSpiceMetabolism()) / 2), 2)} | Decision Model: {agent.decisionModel}"
            else:
